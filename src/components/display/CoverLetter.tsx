...
interface CoverLetterDetailsProps {
  order: Order;
}

const CoverLetterDetails: React.FC<CoverLetterDetailsProps> = ({ order }) => {
  return (
    <div className="bg-gradient-to-br from-teal-50 to-teal-100 p-8 rounded-xl shadow-lg">
      ...
      <div className="space-y-5 text-sm sm:text-base lg:text-xl">
        ...
        <div>
          ...
          <p className='text-sm sm:text-lg lg:text-xl'>{order.companyApplyingTo || <span className="italic text-gray-500">No additional notes</span>}</p>
        </div>
        
        <div>
          <h4 className="flex items-center gap-2 text-lg lg:text-xl font-semibold text-teal-700">Job Applying For</h4>
          <p className='text-sm sm:text-lg lg:text-xl'>{order.jobApplyingFor || <span className="italic text-gray-500">No additional notes</span>}</p>
        </div>

        <div>
          <h4 className="flex items-center gap-2 text-lg lg:text-xl font-semibold text-teal-700">
            <FiUpload className="text-teal-500 lg:text-xl" /> Uploaded Resume
          </h4>
          {order.resumeFile? (
        ...
        <div>
          <h4 className="flex items-center gap-2 text-lg lg:text-xl font-semibold text-teal-700">
            <FiBookOpen className="text-teal-500 lg:text-xl" /> Additional Notes
          </h4>
